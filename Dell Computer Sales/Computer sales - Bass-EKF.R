# ----------------------------------------------------------------------
# Forecasting Computer Sales Using the Time Varying Bass Diffusion Model
# Parameters are Updated Using the Extended Kalman Filter with 
# Continuous States and Discrete Observations
# ----------------------------------------------------------------------
# In this script, we generate Dell computer sales using the Bass-Extended Kalman filter with continuous state and discrete observations method.

# Load in the lubridate and nleqslv package
library(lubridate)
library(deSolve)
source('Bass.R') # Load in functions for the Bass model.
options(warn=-1)
...
for (j in 1:length(priors_list_unique)) {
  ...
  for (i in 1:length(n1)) {
    y1 <- as.numeric(timeSeriesDell[n1[i], ])
    y <- as.numeric(na.omit(y1))  
...
for (i in 1:170) {
  ...
  # These changing points will divide the time series into several periods
  for (j in 0:(H1)) {
    n_prior <- which((end < start[i]+days(7*(j-1))))
    if (length(n_prior) < 2) next
  ...
  # For each period between two changing points, generate forecasts from the Bass-EKF model. The prior distributions stay the same within each period.
  for (aaa in 2:(length(n_change)+1)) {
    ...
    n_prior <- which((end < start[i]+days(7*(nn-1))))
    if (length(n_prior) < 2) next
    ...
    for (t in 1:(nn+1)) {
      ...
      times <- seq(c(t-1+start_t), c(t+start_t), by = 0.1)
      ntime <- 1/0.1+1
      out <- ode(func = modelFunction, y = yini, parms = pars, times = times,method='rk')
      # If ntime is too small, rerun the ode function with a larger ntime
      if (any(out[,2] == -Inf)|is.na(out[ntime,2])|any(out[,2]>10000)) {
      ...
      if (t-1 >= n_change[aaa-1]) {
        ...
        for (j in 2:step) {
          ...
          times <- seq(c(t+start_t+j-2), c(t+start_t+(j-1)), by = 0.1)
          ntime <- 1/0.1+1
          out <- ode(func = modelFunction1, y = yini, parms = pars, times = times,method='rk')
          if (is.na(out[ntime,2])|any(out[,2]>10000|out[ntime,2]< -10000)) {
            times <- seq(c(t-2+start_t+j), c(t+start_t+(j-1)), by = 0.001)
            ntime <- 1/0.001+1
            out <- ode(func = modelFunction1, y = yini, parms = pars, times = times,method='rk')
          }
          if (out[ntime,2] == Inf) ntime <- 1
          npred[j] <- out[ntime,2]
          speriodpred[j] <- npred[j] - npred[j-1]
          for (a in 1:99) {
            speriodpred_q[j,a] <- speriodpred[j] + qnorm(a/100)*sqrt(2)*sqrt(K)
          }
          speriodpred_q[speriodpred_q<0] <- 0 # replace negative predictions with zeros
          speriodpred[speriodpred<0] <-0
        }
