...
for (j in 1:length(priors_list_unique)) {
  ...
  set.seed(201)
  if (model_select == 'QRF') {
    qrf  <- quantregForest(training_data[,1:window], training_data[,window+1],ntree=250)
...
accuracy_result <- accuracy_result[,1:202,,]
...
accuracy_train <- array(NA,dim = c(161,202,24,99))
accuracy_test <- array(NA,dim = c(161,202,24,99))
for (i in 1:161) {
  ...
  if (this_end_train <= 1) {
    ...
    accuracy_test[i,this_end_train:202,,] <- accuracy_result[i,this_end_train:202,,] 
  }
}

result_test <- t(2*c(mean(rowMeans(apply(accuracy_test[,,1:12,50],c(1,3),mean,na.rm=TRUE), na.rm=TRUE),na.rm=TRUE),
                     mean(rowMeans(apply(apply(accuracy_test[,,1:12,1:99],c(1,2,3),mean),c(1,3),mean,na.rm=TRUE), na.rm=TRUE),na.rm=TRUE),
                     mean(rowMeans(apply(accuracy_test[,,13:24,50],c(1,3),mean,na.rm=TRUE), na.rm=TRUE),na.rm=TRUE),
                     mean(rowMeans(apply(apply(accuracy_test[,,13:24,1:99],c(1,2,3),mean),c(1,3),mean,na.rm=TRUE), na.rm=TRUE),na.rm=TRUE)))
colnames(result_test) <- c("1-12 steps MAE","1-12 steps MCRPS","13-24 steps MAE","13-24 steps MCRPS")
result_test
