...
candidateForecasts <- matrix(NA,161, 209)
sigma2Values <- rep(NA,161)
...
for (j in 1:161) {
  ...
  # Generate forecasts
  fitModel$y <- NULL
  if (model_select == 'Bass')  forecasts_j <- forecast.Bass(fitModel,209,level = 99.9)$predictions
  if (model_select == 'GSG')  forecasts_j <- forecast.GSG(fitModel,209,level = 99.9)$predictions
  if (model_select == 'Trap')  forecasts_j <- forecast.Trapezoid(fitModel,209,level = 99.9)$predictions
  if (model_select == 'TiGoTI')  forecasts_j <- forecast.TiGo.ETS(fitModel,209,level = 99.9)$predictions[,-c(2,3)]
  candidateForecasts[j,1:209] <- forecasts_j[,1] # Store point forecasts to candidateForecasts
