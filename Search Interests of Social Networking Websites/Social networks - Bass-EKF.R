...
# In this script, we generate search interests of social networking websites using the Bass-Extended Kalman filter with continuous state and discrete observations method.
...
for (j in 1:length(priors_list_unique)) {
  ...
  for (i in 1:length(n1)) {
    y1 <- as.numeric(timeSeriesSearchInterests[,n1[i]])
    y <- as.numeric(na.omit(y1))  
...
for (i in 1:161) {
  ...
  for (aaa in 2:(length(n_change)+1)) {
    ...
    n_prior <- which((end < start[i]+months(nn-1)))
    if (length(n_prior) < 2) next
    ...
    for (t in 1:(nn+1)) {
      ...
      if (t-1 >= n_change[aaa-1]) {
        ...
        speriodpred_q[speriodpred_q>100] <- 100 # replace forecasts > 100 with 100
