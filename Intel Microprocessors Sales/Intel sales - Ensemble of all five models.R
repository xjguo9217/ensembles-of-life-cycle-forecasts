...
# Using Bayesian Ensemble of All Five Model Types 
# (Including the TiGo-ETS model)
# ------------------------------------------------

# we generate Bayesian ensemble forecasts of Intel microprocessors sales.
...
# For each of the 86 time series, then generate 187 steps ahead forecasts (the max length of the time series in the data set is 187) from the four time invariant models from time 0. Since these models are time invariant, 1-h step-ahead forecasts generated at time t are equal to t+1 - t+h step-ahead forecasts generated at time 0. For the TiGo-ETS model, we will update states and generate forecasts later.
...
for (i in 1:86) {
  ...
  for (t in 0:H1) { 
    ...
    for (kk in 1:length(n_complete)) {
      ...
      # median and variance used to calculate the likelihood of the on-going life cycle's past data according to candidate model kk.
