...
# Using the Bayesian Ensemble of Time-invariant Diffusion Models
# (Single Model Type)
# ---------------------------------------------------------------
...
candidateForecasts <- matrix(NA,86, 187)
sigma2Values <- rep(NA,86)
...
for (j in 1:86) {
  ...
  # Generate forecasts
  fitModel$y <- NULL
  if (model_select == 'Bass')  forecasts_j <- forecast.Bass(fitModel,187,level = 99.9)$predictions
  if (model_select == 'GSG')  forecasts_j <- forecast.GSG(fitModel,187,level = 99.9)$predictions
  if (model_select == 'Trap')  forecasts_j <- forecast.Trapezoid(fitModel,187,level = 99.9)$predictions
  if (model_select == 'TiGoTI')  forecasts_j <- forecast.TiGo(fitModel,187,level = 99.9)$predictions
  candidateForecasts[j,1:187] <- forecasts_j[,1] # Store point forecasts to candidateForecasts
